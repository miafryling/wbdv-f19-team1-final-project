...
class PublicProfile extends React.Component {
    ...
    componentWillReceiveProps(nextProps, nextContext) {
        this.userService.findUserById(nextProps.match.params.userId)
            .then(user => this.setState({user}));
    }
            ...
            <div className="container">
                ...
                <div className="row">
                    <div className="col-sm-6">
                        <h4>Liked Animals</h4>
                        {
                            this.state.user.animals.map(x => <div className="row"><Link to={`/animals/${x}`}>{x}</Link>
                            </div>)
                        }
                    </div>
                    <div className="col-sm-6">
                        <h4>Friends</h4>
                        {
                            this.state.user.friends.map(x => <div className="row"><Link to={`/users/${x}`}>{x}</Link>
                            </div>)
                        }
                    </div>
                </div>
